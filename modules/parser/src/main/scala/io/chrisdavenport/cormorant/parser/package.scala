...
package object parser {
  ...
  def parseRows(
    ...
    parser.fileBody
      ...
      .map {
        ...
        case rows @ CSV.Rows(CSV.Row(x) :: _) if cleanup && x.size > 1 =>
          filterLastRowIfEmpty(rows)
  ...
  private def filterLastRowIfEmpty(rows: CSV.Rows): CSV.Rows = {
    rows.rows.reverse match {
      case x :: xl if x == CSV.Row(NonEmptyList(CSV.Field(""), Nil)) =>
        CSV.Rows(xl.reverse)
      case _ => rows
    }
  }
