...
trait LabelledWriteProofs extends LowPriorityLabelledWriteProofs {
  ...
  /** Base of Logical Induction for Put Systems.
    *
    * This proves Given a Put Before an HNil, so a single value within a product
    * type. That we can serialize this field by name and value into a CSV
    */
...
private[internal] trait LowPriorityLabelledWriteProofs
  ...
  /** This is the logical extension of the above base induction case Given som
    * Field type with a name we serialize that field as
    */
...
private[internal] trait LowPriorityLabelledWriteProofs1
  ...
  implicit def deriveByLabelledWrite2[K, H, T <: HList](implicit
    ...
    new LabelledWrite[FieldType[K, H] :: T] {
      ...
      def write(a: FieldType[K, H] :: T): CSV.Row = {
        CSV.Row(
          P.write(a.head).l.concatNel(labelledWrite.value.write(a.tail).l)
        )
      }
    }
}
