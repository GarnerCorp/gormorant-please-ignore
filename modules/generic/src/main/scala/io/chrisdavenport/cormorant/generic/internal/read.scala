...
trait ReadProofs extends LowPriorityReadProofs {
  ...
  implicit def readHNil[H](implicit G: Get[H]): Read[H :: HNil] =
    new Read[H :: HNil] {
      def readPartial(
          a: CSV.Row
      ): Either[Error.DecodeFailure, Either[(CSV.Row, H :: HNil), H :: HNil]] =
        a match {
              ...
              case None =>
                Either.left(
                  Error.DecodeFailure.single(
                    s"Unexpected Input: Did Not Expect - $a"
                  )
                )
            }
        }
  ...
  implicit def hlistRead[H, T <: HList](implicit
    ...
    def readPartial(
        a: CSV.Row
    ): Either[Error.DecodeFailure, Either[(CSV.Row, H :: T), H :: T]] =
      a match {
  ...
  implicit def hlistRead2[H, T <: HList](implicit
    ...
    def readPartial(
      ...
      RH.value.readPartial(a).flatMap {
        ...
        case Right(value) =>
          Either.left(
            Error.DecodeFailure.single(s"Incomplete Output - $value only")
          )
      }
  }
}
