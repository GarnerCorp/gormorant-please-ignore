...
package object fs2 {
  ...
  /** Removes any empty row - i.e. Row(NonEmptyList.of(""))
    */
  def parseRowsSafe[F[_]]
      : Pipe[F, String, Either[Error.ParseFailure, CSV.Row]] =
    _.through(
      parseN[F, CSV.Row](opt(CSVParser.PERMISSIVE_CRLF) ~> CSVParser.record)
    )
  ...
  def readRows[F[_]: RaiseThrowable, A: Read]: Pipe[F, String, A] =
    _.through(readRowsSafe).rethrow

  /** Read the first line as the headers, the rest as rows. This is super
    * general to allow for better combinators based on it Removes Empty Rows
    */
  def parseCompleteSafe[F[_]]: Pipe[
    ...
    Either[
      Error.ParseFailure,
      (CSV.Headers, Either[Error.ParseFailure, CSV.Row])
    ]
  ] = {
    ...
    _.through(parse1(opt(CSVParser.PERMISSIVE_CRLF) ~> CSVParser.header))
      .map[Either[Error.ParseFailure, (CSV.Headers, Stream[F, String])]] {
        case (ParseResult.Done(rest, a), s) =>
          Either.right((a, Stream(rest) ++ s))
        case (e, _) =>
          e.either.leftMap(Error.ParseFailure.apply).map(h => (h, Stream.empty))
      }
      .flatMap {
        _.traverse { case (h, s) =>
          s.through(parseN(opt(CSVParser.PERMISSIVE_CRLF) ~> CSVParser.record))
            .map { row =>
              (h, Either.right(row))
            }
        }
  ...
  private def parse1[F[_], A](
      p: atto.Parser[A]
  ): Pipe[F, String, (ParseResult[A], Stream[F, String])] =
    s => {
      def go(
          r: ParseResult[A]
      )(
          s: Stream[F, String]
      ): Pull[F, (ParseResult[A], Stream[F, String]), Unit] = {
        r match {
          case p @ ParseResult.Partial(_) =>
            s.pull.uncons.flatMap {
              // Add String To Result If Stream Has More Values
              case Some((c, rest)) =>
                go(p.feed(Stream.chunk(c).compile.string))(rest)
  ...
  private def parseN[F[_], A](p: Parser[A]): Pipe[F, String, A] =
    s => {
      def exhaust(
          r: ParseResult[A],
          acc: List[A]
      ): (ParseResult[A], List[A]) = {
        r match {
      ...
      def go(r: ParseResult[A])(s: Stream[F, String]): Pull[F, A, Unit] = {
        s.pull.uncons.flatMap {
          case Some((c, rest)) =>
            ...
            Pull.output(Chunk.from(as.reverse)) >> go(r1)(rest)
          case None => Pull.output(Chunk.from(exhaust(r.done, Nil)._2))
        }
      }
      go(p.parse(""))(s).stream
    }
  ...
  def parseComplete[F[_]: RaiseThrowable]
      : Pipe[F, String, (CSV.Headers, CSV.Row)] =
    _.through(parseCompleteSafe).rethrow
      .map { case (h, e) => e.map((h, _)) }
      .rethrow

  def readLabelledCompleteSafe[F[_], A: LabelledRead]
      : Pipe[F, String, Either[Error, A]] =
  ...
  /** Converts the current `Stream` to a `Stream[F, String]` by encoding its
    * content using the provided `Printer`.
    *
    * This method requires a valid `Write[A]` implicit instance.
    *
    * @example
    *   {{{ Stream .emits(list) .through(writeRows(headers, Printer.default))
    *   }}}
    */
  ...
  def encodeWithHeaders[F[_]](
      headers: CSV.Headers,
      p: Printer
  ): Pipe[F, CSV.Row, String] =
    s =>
      Stream(p.print(headers)).covary[F] ++ Stream(p.rowSeparator) ++ s.through(
        encodeRows(p)
      )

  /** Converts the current `Stream` to a `Stream[F, String]` by encoding its
    * content using the provided `Printer` and prepending the provided headers.
    *
    * This method requires a valid `Write[A]` implicit instance.
    *
    * @example
    *   {{{ Stream .emits(list) .through(writeWithHeaders(headers,
    *   Printer.default)) }}}
    */
  def writeWithHeaders[F[_], A: Write](
      headers: CSV.Headers,
      p: Printer
  ): Pipe[F, A, String] =
    s =>
  ...
  /** Converts the current `Stream` to a `Stream[F, String]` by encoding its
    * content using the provided `Printer` and prepending the headers extracted
