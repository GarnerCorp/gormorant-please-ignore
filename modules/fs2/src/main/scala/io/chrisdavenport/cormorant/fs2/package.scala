...
package object fs2 {
  ...
  def encodeWithHeaders[F[_]](headers: CSV.Headers, p: Printer): Pipe[F, CSV.Row, String] =
    s => Stream(p.print(headers)).covary[F] ++ Stream(p.rowSeparator) ++ s.through(encodeRows(p))
   ...
   * }}}
   */
  def writeWithHeaders[F[_], A: Write](headers: CSV.Headers, p: Printer): Pipe[F, A, String] =
    s => Stream(p.print(headers)).covary[F] ++ Stream(p.rowSeparator) ++ s.through(writeRows(p))
